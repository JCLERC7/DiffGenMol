...
class Trainer1D():
    ...
    def __init__(self, data_loader, timesteps, epochs, lr, unet_dim, unet_channels, unet_cond_drop_prob, results_folder, config, save_dir, save_period, tensorboard):
        ...
        self.train_metrics = utils.MetricTracker('loss', 'validity', 'uniqueness','novelty', 'KLdiv', 'similarity', 
                                                 'classes_similarity', 'classes_match', writer=self.writer)
    ...
    def _train_epoch(self, epoch):
        ...
        for step, batch in enumerate(self.data_loader.get_dataloader()):
            ...
            self.writer.set_step((epoch - 1) * self.len_epoch + step)
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            with torch.no_grad():
                # unconditional
                nb_mols = 3
                samples_classes = torch.tensor([0 for i in range(nb_mols)]).to(self.device)
            ...
            self.train_metrics.update('validity', metrics.validity_score(samples_smiles))
            self.train_metrics.update('uniqueness', metrics.uniqueness_score(samples_smiles))
            self.train_metrics.update('novelty', metrics.novelty_score(samples_smiles, self.data_loader.get_train_smiles()))
            self.train_metrics.update('KLdiv', metrics.KLdiv_score(samples_smiles, self.data_loader.get_train_smiles()))
            self.train_metrics.update('similarity', metrics.top3_tanimoto_similarity_score(samples_mols, self.data_loader.get_train_mols()))
